...
class QUIC_CONNECTION:
    ...
    async def send_to_streams(self) -> None:
        ...
        await asyncio.gather(*(self.send_stream_data(stream_id,frame_payload_size,frames_per_packet) for stream_id in self.out_streams))
    ...
    async def receive_data(self) -> List[bytes]:
        ...
        if received_packet.packet_flag != (FLAGS.SYN or
            ...
            if received_packet.packet_flag == FLAGS.FIRST_PACKET:
                self.streams_stats[received_frames[0].stream_id] = Stats(received_frames[0].stream_id,0,0,0,time.time())
                if self.connection_stats[OVERALL_DATA] is None:
            ...
            if received_packet.packet_flag == FLAGS.LAST_PACKET:
                self.streams_stats[received_frames[0].stream_id].time = time.time() - self.streams_stats[received_frames[0].stream_id].time
